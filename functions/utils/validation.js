...
export function validateProduct(product) {
  ...
  if (
    !product.price ||
    !ValidationSchemas.price.test(product.price.toString())
  ) {
  ...
  if (
    ...
    !ValidationSchemas.quantity.test(product.stock_quantity.toString())
  ) {
...
export function validateOrder(order) {
  ...
  if (
    ...
    !["mada", "stc_pay", "stripe", "paypal"].includes(order.payment_method)
  ) {
...
export class RateLimit {
  ...
  async checkLimit(key, limit = 10, windowMs = 60000) {
    ...
    await this.kv.put(`rate_limit:${key}`, JSON.stringify(requests), {
...
export async function hashPassword(password) {
  ...
  const data = encoder.encode(
    password + process.env.SALT_SECRET || "fallback_salt"
  );
